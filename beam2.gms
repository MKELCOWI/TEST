...
* * =============================================================================
* Syntax:
* All variables are written with upper capital letters. Example "IRG(bPlz,j,y,m0) "
* Index variables are denotes with small case letter i and then upper capital letters. Example " iYIELD(bPlz,j,y)"
* All parameters and tables are written with mostly lower case letters. Example "qAWater(bPlz,j)"
* All equations are written with upper case letters. Example "WBALANCE(s,b,y,m).. "
* =============================================================================
...
SETS
    ...
    su(m0)                  "Summer season"         / m04, m05, m06, m07, m08, m09/
    wi(m0)                  "Winter Season"         / m01, m02, m03, m10, m11, m12/
    ...
    l                "Type of loss, either seepage or evaporation " /seep, eva/
...
* Added by MKEL 8.- february 2013. Table plantM(j, m0) Used for defining the months, that we water the plants in.
* We dont have data yet, so this is just a rough guess, based on agriseason table.  Wheat is special because there is no leach zone.

    plantMonth(j, m0)        "Planting Month for Crop j" / cot.m05, cot.m06, cot.m07, cot.m08, cot.m09,
                                                       wht.m04, wht.m05, wht.m06, wht.m10, wht.m11,
                                                       veg.m04, veg.m05, veg.m06, veg.m07, veg.m08, veg.m09,
                                                       fru.m03, fru.m04, fru.m05, fru.m06, fru.m07, fru.m08, fru.m09, fru.m10,
                                                       ric.m04,  ric.m05, ric.m06, ric.m07, ric.m08,
                                                       alf.m04, alf.m05,  alf.m06, alf.m07, alf.m08, alf.m09, alf.m10, alf.m11,
                                                       oth.m05,  oth.m06, oth.m07, oth.m08, oth.m09    /

    leachMonth(j, m0)         "Leaching Month for Crop j" / cot.m01, cot.m02,
                                                            veg.m01, veg.m02,
                                                            ric.m01, ric.m02,
                                                            alf.m01, alf.m02,
                                                            oth.m01, oth.m02
...
POSITIVE VARIABLES
    ...
    iYIELD(b,j,y0)          "Index for yield response to water shortage (1=max yield)"
    iINPUT(b,j,k,y0)        "Index for use of production inputs (1=baseyear)"
    iLAND(b,j,y0)           "Index for physical amount of land"
*IRG variable added by MKEL monday, 4. february 2013
    IRG(b,j,y0,m0)          "Represent irrigation of water in node b, for crop j, at year y, in month m0.(mm3)"
*MKEL monday, 15. february 2013
    LCH(b,j,y0,m0)           "Represent the leaching of water in node b, for crop j, at year y, in month m0.(mm3)"
*MKEL monday, 15. february 2013
   LOSSC(b,l,y0,m0)       "Loss in channels of type l, going to node b, at year y, in month m0. (mm3)"
   LOSSF(b,j, l,y0,m0)    "Loss in fields of type l, going to node b, used for crop j, at year y, in month m0. (mm3)"
   RATE(b,j,y0)
   RATEM(b,j,y0, m0)
   RATE2(b,j,y0, m0)        "TESTING VARIABLE"
   CHECKTEST(b,j,y0, m0)    "TESTING VARIABLE"
   KONSTANT(s,b,y0)         "TESTING VARIABLE"

   iDISCHARGE(bd, bo, y0)    "Relative discharge variable, this is done to fix the rate to historical patterns "
;
...
OPTIONS iLAND:3:1:2;
OPTIONS iYIELD:3:1:2;
OPTIONS CHECKTEST:0:3:1;
...
* Hydrological constraints and identities
    ...
    ELECMINPRODPEAK(c, y0, m)   "Electricity minimum production for each country in peak hours"
    ELECMINPRODBASE(c, y0, m)   "Electricity minimum production for each country in base hours"
...
* Limit the reservoir level manually Added by MKEL tuesday, january 31 2013
    RESVOLCHANGE(c, m, y0, s)      "Change the maximum Reservoir level"
...
*    FAO33(b,j,y0)              "Yield response function, the yield as a function of the irrigation, and optimal water input. Because of NL of problem, perhaps abundant now"
    RATEEQ(b,j,y0, m0)          "Calculate different yieldrates that are possible"
    RATEMAX(b,j,y0, m0)          "The yield should not be bigger than the smallest possible yieldrate"
    YIELDCAL(b,j,y0)             "Calculate the yield based on the rateeq equation of minimum yield"
    FAO33max(b,j,y0)            "Yield response function - you cannot have a yield bigger than one "

    RATEEQ2(b,j,y0,m0)          "Only used for testing purposes"
    CHECK(b,j,y0,m0)            "Only used for testing purposes"
* Next 4 below constraint added by MKEL monday, 11. february 2013
    HELPSOLUTION(b,j,y0, m0)    "Help gams find the solution, saying no single irrigation should be less than 90 % of what is was last year, adjusted with output"
    LEACH(b,j,y0, m0 )          "Leach minimum requirements. Each planning zone is required to leach a minimum of water"
    NOWATER(b,j,y0, m0)         "Some months should be forced to not irrigate at all"
    NOWATERLEACH(b,j,y0, m0)    "Some months should be forced to not leach at all"
* WATER LOSS CONSTRAINTS MKEL 15. februray 2013
    CHALOSS(b,l,y0,m0,s)    "Calculate the loss in fields for each node b, for each type l, for each year y0 in each month m0"
    FIELDLOSS(b,j,l,y0,m)   "Calculate the loss in fields for each node b, to each crop j, for each type l, for each year y0 in each month m0"



*   New equations below only used for various user defined scenarios. MKEL
*   The colume should never change, meaning we have river run off.
    CONSTANTVOLUME(s,b,y0,m0)
    HISTPATTERNS(s,b,b,y0,m0)    "If user choeses, he can limit the power production to be the same pattern as in the year 2009."

    HARM(s,b,b,y0,m0)            "Ensures that there is no flow from a reservoir that has no production."
* Objective functions
...
* Because of industrial demand (qiWater) it looks like its not possible to just let the river run off, since the demand required attention
* this has the effect that we need to lower the volume...
WBALANCE(s,b,y,m)..
    scalem3 * (
   ...
   + STO(s,b,y,m)$sea(b)
   + STO(s,b,y,m)$bRes(b)
    + SUM(j,  IRG(b,j,y,m)$bPlz(b)* (1+SUM(l, lossFields(b, j, l, m)$bPlz(b)  )   )    )  *  (  1-iInv("improve")  )
* New equation in WaterBalance added by MKEL monday, 11. february 2013 below the old one.
*   +e SUM(j, iINPUT(b,j,"watr",y)*qAWater(b,j)*agriSeason(b,j,m))*(1-iInv("improve")) old line
   + SUM(j, LCH(b,j,y,m)$bPlz(b) )
* Added LEACH variable, by MKEL 15. february 2013
   + qIWater(b,m)*(1+demography(y,"Ind"))
   + qHWater(b,m)*(1+demography(y,"HH"))
    )
  =e=
* NOtice now that we do losses on intakes leading to planning zones. However no losses on intakes that doesnt go into planning zones. MKEL
     ( SUM(bo$( intk(b,bo) AND bPlz(b) ), ITK(s,b,bo,y,m)) * (  1-SUM(l, lossChannels(b, l, m))  )
      +SUM(bo$(intk(b,bo) AND NOT bPlz(b) ), ITK(s,b,bo,y,m))
...
* Mavimum reservoir volume set by outer parameters. Added by MKEL 31. January 2013
RESVOLCHANGE(c, m, y0, s)    ..
         SUM(b$(bCty(b, c) and bResEly(b)), VOL(s,b,y0,m) * scalem3) =l= ResMaxCon(c,m);
...
*Minimum Production of electricity during peak hours for each country.
*## Added by MKEL ##
ELECMINPRODPEAK(c, y, m)..
       ElyPeakMinimum(c, m) =l= SUM(b$(bCty(b, c) and bResEly(b)), ELYPEAK(b, y, m));

*Minimum Production of electricity during base hours for each country.
*## Added by MKEL ##
ELECMINPRODBASE(c, y, m)..
       ElyBaseMinimum(c, m) =l= SUM(b$(bCty(b, c) and bResEly(b)), ELYBASE(b, y, m));
...
LEONTIEF(b,j,k,y)$(iShare(b,k,j) and (not kWater(k)))..
...
* Yield response function to water shortages MKEL. This is the non-linear way to do it, which can be done, but does create difficulties.
* Notice if there is no demand for water at all  in a given month, there should be no loss in yield
* - which the line (m0$(plantMonth(j, m0) and agriSeason(bPlz,j,m0) secures.
*FAO33(bPlz,j,y) ..
*    iYIELD(bPlz,j,y) =e=  PROD(m0$(plantMonth(j, m0) and agriSeason(bPlz,j,m0) ) ,
*   1 - jYield(j, m0)*(        1  -  IRG(bPlz,j,y,m0) / (       iOUTPUT(bPlz,j,y) *  qAWater(bPlz,j)*agriSeason(bPlz,j,m0)       )          )
*
*);


* We calculate the minimum of the maximum yield indirectly by doing this MKEL
RATEEQ(bPlz,j,y, m0)$(plantMonth(j, m0) and agriSeason(bPlz,j,m0) )..
* OLD WAY RATE(bPlz,j,y) =l= (IRG(bPlz,j,y,m0) / (       iOUTPUT(bPlz,j,y) *  qAWater(bPlz,j)*agriSeason(bPlz,j,m0)       )    ) ;
RATEM(bPlz,j,y, m0) =e=
1-jYield(j, m0)*(1-
SUM(mm0$( ORD(mm0)<ORD(m0)+1 AND plantMonth(j, mm0)), IRG(bPlz,j,y,mm0)  ) /
SUM(mm0$( ORD(mm0)<ORD(m0)+1 AND plantMonth(j, mm0)),    (  iOUTPUT(bPlz,j,y) *  qAWater(bPlz,j)*agriSeason(bPlz,j,mm0)   )            )
)
;

* The biggest rate (yield), cannot be bigger than the smallest "RATEM". MKEL
RATEMAX(bPlz,j,y, m0)..
RATE(bPlz,j,y) =l= RATEM(bPlz,j,y, m0) ;

* Calculate the yield as a function of the minimum rate....
YIELDCAL(bPlz,j,y) ..
iYIELD(bPlz,j,y) =e=RATE(bPlz,j,y);
*iYIELD(bPlz,j,y) =e= 1-jYield(j, m0)*(1-RATE(bPlz,j,y));
*iYIELD(bPlz,j,y) =e= 1-jYield(j)*(1-RATE(bPlz,j,y));

* Constrain water input so that extra water above max water need does not produce additional crops.  MKEL
FAO33max(bPlz,j,y)$qInputs(bPlz,"watr",j)..
    iYIELD(bPlz,j,y) =l= 1;

*Limit the solution, to thereby indirectly help the non linear solver guess the optimal irrigation,
*not used with new calculation of yield.  Added by MKEL 11. february 2013.
*HELPSOLUTION(bPlz,j,y, m0 )$plantMonth(j, m0)..
*IRG(bPlz,j,y,m0) =g=    iOUTPUT(bPlz,j,y)* qAWater(bPlz,j)*agriSeason(bPlz,j,m0)  *0.9;


*THIS IS A TEST used for checking calcucaltions etc.
*Do this for all planting months, and where we know its different from zero, just to be safe.
* Sum op all the planting months up to m0. Compare with check test, to see we do everthing correctly.
RATEEQ2(bPlz,j,y, m0)$(plantMonth(j, m0) AND agriSeason(bPlz,j,m0) )..
RATE2(bPlz,j,y, m0)  =e=
SUM(mm0$( ORD(mm0)<ORD(m0)+1  AND plantMonth(j, mm0)),    (  iOUTPUT(bPlz,j,y) *  qAWater(bPlz,j)*agriSeason(bPlz,j,mm0)   )            )
*SUM(mm0$( ORD(mm0)<ORD(m0)+1  AND plantMonth(j, mm0)),    (  iOUTPUT(bPlz,j,y) *  qAWater(bPlz,j)*agriSeason(bPlz,j,mm0)   )            )
;

*THIS IS A TEST for checking calcucaltions etc.
*Compare this with to RATE2 or IRG, to make sure we sum op correctly
CHECK(bPlz,"wht",y, m0)..
CHECKTEST(bPlz,"wht",y, m0) =e=
*General test SUM(mm0$( ORD(mm0)<ORD(m0)+1 AND plantMonth(j, mm0)), IRG(bPlz,j,y,mm0)  )
SUM(mm0$( ORD(mm0)<ORD(m0)+1 AND plantMonth("wht", mm0) AND ORD(mm0)< 10), IRG(bPlz,"wht",y,mm0)  )  + SUM(mm0$(plantMonth("wht", mm0) AND ORD(mm0)> 9), IRG(bPlz,"wht",y,mm0)  )
*(iOUTPUT(bPlz,j,y) *  qAWater(bPlz,j)*agriSeason(bPlz,j,m0) )
;


*Define that leaching months are required, and are a function of the output for previous years. Added by MKEL 15. february 2013. RHS should be changes to parameter (?)
LEACH(bPlz,j,y, m )$leachMonth(j, m) ..
*MKEL 15.february 2013. Could use a lchNormal coefficent instead of * 1...
*LCH(bPlz,j,y,m) =e=   iOUTPUT(bPlz,j,y)*   (     qAWater(bPlz,j)*agriSeason(bPlz,j,m) * 1   ) ;
1 =e= 1;


*Some months, should not be irrigated at all.
NOWATER(bPlz,j,y, m0)$(agriSeason(bPlz,j,m0) = 0 OR leachMonth(j, m0))..
IRG(bPlz,j,y,m0) =e= 0;

*Some months, should not be leached at all.
NOWATERLEACH(bPlz,j,y, m0)$(NOT leachMonth(j, m0) ) ..
LCH(bPlz,j,y,m0) =e= 0;

* Loss of water in the channels when leading down to a planning zone.
CHALOSS(bPlz, l, y0, m, s)..
LOSSC(bPlz,l,y0,m) =e= SUM(bo, ITK(s,bPlz,bo,y0,m)$intk(bPlz,bo)  *  lossChannels(bPlz, l, m) );

* Loss of water in the fields when irrigating
FIELDLOSS(bPlz, j, l, y0, m)..
LOSSF(bPlz,j, l,y0,m) =e= IRG(bPlz,j,y0,m)*  lossFields(bPlz, j, l, m);


******** NEW CONSTRAINTS 25 february**********
*   The volume should never be bigger, meaning we have river run off, since you are not allowed to store any water.
*   There is a problem, in order to make this work. NOT WORKING    NOT WORKING  NOT WORKING
CONSTANTVOLUME(s,b,y0, m0)$(scUser("runoff") AND bResBuild(b)  AND ORD(m0) > 1 AND ORD(m0) < 4)..
*VOL(s,b,y0,m0)$bRes(b) =e= SUM(mm$(bRes(b) AND ORD(mm) = ORD(m0) -1 ),  VOL(s,b,y0,mm)) ;
STO(s,b,y0,m0) =e= SUM(bd$resv(bd,b), DIS(s,bd,b,y0,m0) );
*1 =e= 1;
*STO(s,b,y0,m0) =e= 0;

* Discharges should equal historical patterns for NUREK and TOKTUGUL WORKING.
HISTPATTERNS(s,bd,bo,y0,m)$(scUser("hist") AND rDisBas0(bd,bo,y0,m))..
*absolutte pattern
*rDisBas0(bd,bo,y0,m) =e= DIS(s,bd,bo,y0,m);
*relative pattern
rDisBas0(bd,bo,y0,m) * iDISCHARGE(bd, bo, y0) =e= DIS(s,bd,bo,y0,m);


******** NEW CONSTRAINTS 4 marts Note 2.4**********   NOT WORKING    NOT WORKING       NOT WORKING
* Constraint that ensures that no waterflow is less than the discharge for nodes b, that are reservoirs. This ensures that
* in the case, where user opts to shut down production, he also shuts down the waterflow ! Perhaps this could also be zero?
HARM(s,bd,b,y,m)..
1 =e= 1;
*VOL(s,b,y,m)
*SUM(mm, VOL(s,b,y0,mm)$(ORD(mm) = ORD(m0)-1)  ) ;
*    20000 =g= STO(s,b,y0,m0);
*1 =e= 1;
...
*weight over single months, and use value for objective, that the user has choosen.
*                   +SUM((b,y,m)$bResEly(b), ELYBASE(b,y,m) * pEly(y,m) / 1000000    *   weightUserMonth("MaxHEPSwin", m) * weightUserMonth("MaxHEPSann", m) *  weightUserMonth("equal", m)                         )
*                   +SUM((b,y,m)$bResEly(b), ELYPEAK(b,y,m) * pEly(y,m)*elyPeakFac / 1000000   *   weightUserMonth("MaxHEPSwin",m) * weightUserMonth("MaxHEPSann",m) *  weightUserMonth("equal", m)                  )


*winter: only sum over the winter months, and use value for objective, that the user has choosen.
                   +SUM((s,bd,b,y,m)$(wi(m) AND bRes(b)), DIS(s,bd,b,y,m) + FLW(s,bd,b,y,m)  )
                         *   weightUser("MaxFlowWin") * weightUser("MaxFlowAnn") *  weightUser("NoWeight")
*summer: only sum over the summer months, and use value for objective, that the user has choosen.
                   +SUM((s,bd,b,y,m)$(su(m) AND bRes(b)), DIS(s,bd,b,y,m) + FLW(s,bd,b,y,m)  )
                         * weightUser("MinFlowSum") *  weightUser("MaxFlowSum") * weightUser("MaxFlowAnn") *  weightUser("NoWeight")
                   ...
                   -SUM((b,j,y, m), iInv("usdPerHa")*iINPUT(b,j,"land",y)*qALand(b,j) + (1-iInv0("improve"))*iInv("usdPerMM3")*IRG(b,j,y,m))/1000000
;


MODEL beam  /   WBALANCE, RESVOL, RESFIXVOL, RESFIXDIS, SEAVOL, RESDISMAX, ELECHEAD, RESVOLMAX, RESVOLMIN, RESVOLCHANGE,
                ELECBASE, ELECPEAK,  ELECSALES, ELECMINPRODPEAK, ELECMINPRODBASE, GRNDWATER, MINOUTFLOW, MININFLOW, MININFLOWa,
                LEONTIEF, CETLAND, FIXEDLAND, YIELDCAL, RATEEQ, RATEMAX, RATEEQ2,  CHECK, FAO33max, LEACH, NOWATER, NOWATERLEACH,
                CHALOSS, FIELDLOSS, CONSTANTVOLUME, OBJ, HISTPATTERNS, HARM /;

MODEL beamBASE  /   WBALANCE, RESVOL, RESFIXVOL, RESFIXDIS, SEAVOL, RESDISMAX, ELECHEAD, RESVOLMAX, RESVOLMIN,
                ELECBASE, ELECPEAK,  ELECSALES, GRNDWATER, MINOUTFLOW, MININFLOW, MININFLOWa,
                LEONTIEF, CETLAND, FIXEDLAND, YIELDCAL, RATEEQ, RATEMAX, RATEEQ2, CHECK, FAO33max, LEACH, NOWATER, NOWATERLEACH,
                CHALOSS, FIELDLOSS, CONSTANTVOLUME, OBJ, HISTPATTERNS, HARM /;
...
* Baseline Levels / Initial aproximate values
IRG.fx(bPlz,j,y,m0)$(agriSeason(bPlz,j,m0) = 0 )   = 0;
...
iYIELD.l(b,j,y)                 = 1$qAQuant(b,j);
COSTS.l(b,j,y)                  = SUM(k$(not kLimit(k)), iACosts(b,k,j));

* Added by MKEL 11 february 2013, this alternative starting guess, will find a decent local maximum and not a global.
*IRG.l(b,j,y,m0)                 = iOUTPUT.l(b,j,y) *qAWater(b,j)*agriSeason(b,j,m0);
...
*   MKEL  22 February 2013  Added irrigation, leaching and losses to be shown in Excel output...
        Irrigation      "Irrigation of water (mm3/year). Note Before Loss due seepage and evaporation",
        Leaching        "Leaching of water (mm3/ year)"/
...
*   MKEL  22 February 2013  Added ChannelLoss and Fielsloss to be shown in Excel output...
        ChannelLoss     "Loss in channels  (mm3/ year)",
        FieldLoss       "Loss in fields(mm3/ year)"/
...
*addedlimit time used to solving model. MKEL.
*TO DO : lessen tolerance for finding a solution.
beamBase.reslim = 90;
beamBase.optfile = 1;
beamBase.solprint = no;
SOLVE beamBASE MAXIMIZING twv USING NLP;
*abort "Test baseline only";
modlStatB = beamBASE.modelstat;
solvStatB = beamBASE.solvestat;
WBALANCE.M("w",b,y,m)$(WBALANCE.M("w",b,y,m) eq eps) = 0;
DISPLAY IRG.L,  iOUTPUT.l, iYield.L, RATEM.L, RATE.L;
...
*line below replaced SUM(j,            -iINPUT.l(b,j,"watr",y)*qAWater(b,j)*agriSeason(b,j,m))  with - sum ( IRG.L(b,j,y,m)  + LCH.L(b,j,y,m  )  MKEL
*rFlowBase(b,"OutDem",y,m)                 = SUM(j,            -iINPUT.l(b,j,"watr",y)*qAWater(b,j)*agriSeason(b,j,m))*(1-iInv0("improve")) - qIwater(b,m) - qHwater(b,m);
rFlowBase(b,"OutDem",y,m)                 = SUM(j,            -(IRG.L(b,j,y,m)  + LCH.L(b,j,y,m)) )*(1-iInv0("improve")) - qIwater(b,m) - qHwater(b,m);
...
* Old line below. Replaced by   MKEL  22 February 2013 . Wateruse is now Irrigation + LCH
*rAgriBase(b,"WaterUse",j,y)                   = iINPUT.l(b,j,"watr",y)*qAWater(b,j)*(1-iInv0("improve"));
rAgriBase(b,"WaterUse",j,y)                  = SUM(m, IRG.L(b,j,y,m)+LCH.L(b,j,y,m))*(1-iInv0("improve"));
...
* Old line, replaced iInput.l*qAwater with Sum(m, IRG+LCH)  MKEL  22 February 2013
...
rAgriBase(b,"IrrInvCost",j,y)                 = -1000*(iInv("usdPerHa")*iINPUT.l(b,j,"land",y)*qALand(b,j) + (1-iInv("improve"))*iInv("usdPerMM3")*SUM(m, IRG.L(b,j,y,m)+ LCH.L(b,j,y,m))    )/1000000 ;
...
* Old water use redefined as part of irrigation    MKEL  22 February 2013
rAgriBase(b,"Irrigation",j,y)                 = SUM(m, IRG.L(b,j,y,m) )     *(1-iInv("improve"));
rAgriBase(b, "Leaching", j, y)                = SUM(m, LCH.L(b,j,y,m));
...
*  MKEL  22 February 2013
rEconBase(b, "ChannelLoss", y, m)             = SUM(l, LOSSC.L(b,l,y,m) );
rEconBase(b, "FieldLoss", y, m)               = SUM((l, j), LOSSF.L(b,j ,l,y,m) );
...
*added simple stuff to work with MKEL
beam.reslim = 90;
beam.optfile = 1;
beam.solprint = yes;
...
*line below replaced SUM(j,            -iINPUT.l(b,j,"watr",y)*qAWater(b,j)*agriSeason(b,j,m))  with - sum ( IRG.L(b,j,y,m)  + LCH.L(b,j,y,m  )  MKEL  22 February 2013
*rFlowBase(b,"OutDem",y,m)                 = SUM(j,            -iINPUT.l(b,j,"watr",y)*qAWater(b,j)*agriSeason(b,j,m))*(1-iInv0("improve")) - qIwater(b,m) - qHwater(b,m);
rFlowCtrf(b,"OutDem",y,m)                 = SUM(j,            - (IRG.L(b,j,y,m)  + LCH.L(b,j,y,m)) )*(1-iInv0("improve")) - qIwater(b,m) - qHwater(b,m);
...
* Old line below. Replaced by MKEL. Wateruse is now Irrigation + Leaching     22 February 2013
*rAgriCtrf(b,"WaterUse",j,y)                   = iINPUT.l(b,j,"watr",y)*qAWater(b,j)*(1-iInv0("improve"));
rAgriCtrf(b,"WaterUse",j,y)                   = SUM(m, IRG.L(b,j,y,m)+LCH.L(b,j,y,m))*(1-iInv0("improve"));
rAgriCtrf(b,"WaterSave",j,y)                  = SUM(m, IRG.L(b,j,y,m)+LCH.L(b,j,y,m)) *iInv0("improve");
...
* Old line, replaced iInput.l*qAwater with Sum(m, IRG+ LCH.L(b,j,y,m)) MKEL    22 February 2013
*rAgriCtrf(b,"IrrInvCost",j,y)                 = -1000*(iInv("usdPerHa")*iINPUT.l(b,j,"land",y)*qALand(b,j) + (1-iInv("improve"))*iInv("usdPerMM3")*iINPUT.l(b,j,"watr",y)*qAWater(b,j))
rAgriCtrf(b,"IrrInvCost",j,y)                 = -1000*(iInv("usdPerHa")*iINPUT.l(b,j,"land",y)*qALand(b,j) + (1-iInv("improve"))*iInv("usdPerMM3")*SUM(m, IRG.L(b,j,y,m)+ LCH.L(b,j,y,m))  )/1000000 ;
...
rAgriCtrf(b,"Irrigation",j,y)                 = SUM(m, IRG.L(b,j,y,m) )     *(1-iInv("improve"));
rAgriCtrf(b, "Leaching", j, y)                = SUM(m, LCH.L(b,j,y,m));
...
* Added by MKEL 22 February 2013
rEconCtrf(b, "ChannelLoss", y, m)             = SUM(l, LOSSC.L(b,l,y,m) );
rEconCtrf(b, "FieldLoss", y, m)               = SUM((l, j), LOSSF.L(b,j ,l,y,m) );
...
* Just a dummy variable to test...
parameter
t(b,j,y0);
t(b,j,y0) = IRG.L(b,j,y0, "m01");
...
DISPLAY minInflow0, natExtra, STO.l, VOL.L , FLW.l, ITK.l, ELYBASE.L,  ELYPEAK.L, Dis.L, iYIELD.L;
   ;