...
SETS
    ...
    sSce                    "Scenarios"             / base, ctrf /
     typ                     "Type of loss, either seepage or evaporation " /seep, eva/
;
...
PARAMETERS
    ...
    lossChannels(typ, m)    "Loss coefficent for channels for each month for different types typ"

* ### Yield response coefficients - below is dummy data   MKEL 4. february 2013
    jYield(j) / cot 1.1, wht 0.9, alf 1, ric 1, fru 1, veg 1, oth 1 /;
...
*   New variable used for irrigation by MKEL 4- february 2013
*    IRG(b,j,y0,m0)            "Index for use of amount of water, in each year y, month m, to body b, for crop j."

    iLAND(b,j,y0)           "Index for physical amount of land"
* Added my MKEL 4. february 2013
    iYIELD(b,j,y0)          "Index for yield response to water shortage (1=max yield)"
VARIABLES
    TWV                     "Total water value (million USD)"
...
*  ELECMINPROPEAK Added by MKEL tuesday, january 29 2013
    ELECMINPRODPEAK(c, m, y0)       "Electricity minimum production for each country in peak hours"
* ^ELECMINPRODBASE Added by MKEL tuesday, january 29 2013
    ELECMINPRODBASE(c, m, y0)       "Electricity minimum production for each country in base hours"
...
* Calculate yield response. Added by MKEL
FAO33(b,j,y0)                    "Yield response function to water shortages"
* Added by MKEL
FAO33max(b,j,y0)            "Yield response function - max yield and water"
*
test(b,j,y0, m0)                "test t test"
...
ELECPEAK(b,y,m)..
*$bResEly(b)..  Should this be deleted  - MKEL (?)
...
* Yield response function to water shortages
FAO33(b,j,y)$qInputs(b,"watr",j)..
    iYIELD(b,j,y) =e= 1 - jYield(j)*( 1 - iINPUT(b,j,"watr",y) / iOUTPUT(b,j,y) );

* Constrain water input so that extra water above max water need does not produce additional crops
FAO33max(b,j,y)$qInputs(b,"watr",j)..
    iYIELD(b,j,y) =l= 1;

*test mKEL
test(bPlz,j,y, m0) ..
iINPUT(bPlz,j,"watr",y)  =e=(  iOUTPUT(bPlz,j,y)   ) ;
...
MODEL beam  /   WBALANCE, RESVOL, RESFIXVOL, RESFIXDIS, SEAVOL, RESDISMAX, ELECHEAD, RESVOLMAX, RESVOLMIN, RESVOLCHANGE,
                ELECBASE, ELECPEAK, ELECSALES, GRNDWATER, ELECMINPRODPEAK, ELECMINPRODBASE,
...
* Leave out electricity minimum constrains for base line scenario, added by MKEL.
...
*Fix the most important resovoir in Syrdarya basin and test, weather it matters
*that we empty it, so that the volume is close to minimum in feb
*Vol.fx("W", "Res_TOK", "2009", "m02") = 5600;
*Fix the most important resovoir in Amydarya basin and test, weather it matters,
*that we empty it, so that the volume is close to minimum in feb
*Vol.fx("W", "Res_NUR", "2009", "m02") = 2800;
*MKEL - 31. Januar - Results show that emptying the reservoirs in winter in one country does not affect other countries.
Vol.fx("W", "Res_NUR", "2009", "m07") = 4100;
